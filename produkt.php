...
$result = mysqli_query($conn, "SELECT * FROM producenci INNER JOIN
    (SELECT IDroweru, NazwaRoweru, OpisRoweru, IDproducenta, CenaJednostkowa, ZdjecieRoweru, NazwaKategorii FROM rowery INNER JOIN kategorierowerow on rowery.IDkategorii = kategorierowerow.IDkategorii ) AS szczegoly
ON producenci.IDproducenta = szczegoly.IDproducenta WHERE IDroweru = ".$page);
...
<html lang="en">
<head>
  <link rel="stylesheet" type="text/css" href="style_produkt.css?v=<?php echo $timestamp;?>">
  <meta charset="UTF-8" />
...
<div id="content">
  <?PHP
    while($row = mysqli_fetch_assoc($result)) {
      ...
      echo "<div class='product_name'>" . $row['NazwaProducenta'] . " " . $row['NazwaRoweru'] . " <br/>" .
        $row['CenaJednostkowa'] . " zł" . "<div class='button'><div class='button_text'> <a href='produkt.php?ID=".$row['IDroweru']."&add_cart=".$row['IDroweru']."' class='fill-div'>
      ...
      echo "<div class='product_info'>
                Kategoria roweru: <a href='sklep.php#'>" . $row['NazwaKategorii'] . "</a><br/>
                Producent: <a href='sklep.php#'>" . $row['NazwaProducenta'] . "</a><br/>
            </div>";
      echo "</div></div>";
      echo '<h3 style="text-align: left; margin: 0; margin-left: 1em; margin-top: 1em;"> Opis Produktu: </h3>
        <div class="product_description">
            <h5 style="margin:0;">'.$row['OpisRoweru'] . '</h5></div>';
    }
    ?>
  </div>
</body>
