...
$result = mysqli_query($conn, "SELECT * FROM sprzedaze WHERE IDklienta = ".$_SESSION['user_id']." ORDER BY DataSprzedazy DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <link rel="stylesheet" type="text/css" href="style_zamowienia.css?v=<?php echo $timestamp;?>">
    <meta charset="UTF-8" />
...
<div id="content">
    ...
    if(mysqli_num_rows($result) >= 1) {
        while ($row = mysqli_fetch_assoc($result)) {
                $date = date_create($row['DataSprzedazy']);
                echo "<div class='order' onclick='window.location.href =\"zamowienie.php?ID=".$row['IDsprzedazy']."\";'>
                <div class='order_details'>
                    <div class='text_container'>
                        ZAM/" . $row['IDsprzedazy'] . "/" . date_format($date, "m/Y") . "<br/>
                        " . date_format($date, "d F Y") . " </br>
                    </div>
                </div>
                <div class='order_products'>";
                    $details_result = mysqli_query($conn, "SELECT * FROM szczegolysprzedazy INNER JOIN  rowery ON szczegolysprzedazy.IDroweru = rowery.IDroweru INNER JOIN producenci ON rowery.IDproducenta = producenci.IDproducenta
                                                         WHERE IDsprzedazy = " . $row['IDsprzedazy'] ." LIMIT 3");
                    if(mysqli_num_rows($details_result) > 1){


                    while ($detail_rows = mysqli_fetch_assoc($details_result)) {
                        echo "<img src='product_photos/" . $detail_rows['IDroweru'] . ".jpg' class='image' alt='product_photo'></img>";
                    }
                    }else{
                        $detail_rows = mysqli_fetch_assoc($details_result);
                        echo "<img src='product_photos/" . $detail_rows['IDroweru'] . ".jpg' class='image' alt='product_photo'></img>
                              <h2 style='height: fit-content;width: fit-content;position: relative;float: left;top: 5em;font-weight: normal;color: rgb(112, 112, 112);'>".$detail_rows['NazwaProducenta']." ".$detail_rows['NazwaRoweru']."</h2>";
                    }
                echo "</div>
              </div>";
        }
    }else{ echo "<h2>Jeszcze nic nie zamówiłeś!</h2><br/> <a href='sklep.php'>Znajdź coś dla siebie</a>"; }
?>
</div>
</body>
<script>
