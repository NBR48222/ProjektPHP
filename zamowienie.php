...
<html lang="en">
<head>
  <link rel="stylesheet" type="text/css" href="style_zamowienie.css?v=<?php echo $timestamp;?>">
  <meta charset="UTF-8" />
...
<div id="content">
<?php
    $user = mysqli_query($conn,"SELECT * FROM klienci
            INNER JOIN sprzedaze ON sprzedaze.IDklienta = klienci.IDklienta
            WHERE klienci.IDklienta =".$_SESSION['user_id']);
    $user_name = mysqli_fetch_assoc($user);
    $date = date_create($user_name['DataSprzedazy']);
    echo "<div class='order_info'>
            <div class='order_info__top'>
            <h3>ZAM/" . $user_name['IDsprzedazy'] . "/" . date_format($date, "m/Y") . "</h3></div>
            <div style='padding:1em;'>". $user_name['ImieKlienta']." ".$user_name['NazwiskoKlienta'] ."
            <br/>".$user_name['MiastoKlienta']."<br/>".$user_name['UlicaKlienta'] ." ".$user_name['NrDomuKlienta']."
            <br/>" . date_format($date, "d F Y") . "
    </div></div>";
?>
  <h3 style="text-align: left; margin: 0; margin-left: 1em; margin-top: 1em;"> Zamówione produkty: </h3>
  <?PHP
  $summary=0;
  while($row = mysqli_fetch_assoc($result)) {
    $producent = mysqli_query($conn, "SELECT NazwaRoweru, NazwaProducenta FROM producenci INNER JOIN rowery ON rowery.IDproducenta = producenci.IDproducenta
                                            WHERE NazwaRoweru = '".$row['NazwaRoweru']."'");
    $producent_name = mysqli_fetch_assoc($producent);
    echo "<div class='cards'>";
    $img = $row['ZdjecieRoweru'];
    //                        file_put_contents('product_photos/'.$row['IDroweru']. '.jpg', $img);
    echo "<img src='product_photos/" . $row['IDroweru'] . ".jpg' class='image' alt='product_photo'>";
    echo "<div class='text_container'>";
    echo "<div class='product_name'>" . $producent_name['NazwaProducenta'] . " " . $row['NazwaRoweru'] . " <br/>" .
      $row['CenaJednostkowa'] . " zł" . " Ilość: ".$row['Ilosc']."
            </div> <p style='text-align:center;'>Łącznie: ". $row['Ilosc']*$row['CenaJednostkowa']." zł</p></div>";
    echo "</div>";
    $summary+=($row['Ilosc']*$row['CenaJednostkowa']);
  }
  echo "<div class='summary'><h3 style='text-align:right; margin-bottom:0; margin-right:2em; margin-top: 1em;'> Razem: </h3><br/>
        <p style='margin-right: 1em;text-align: right; margin-bottom: 1em;'>".$summary." zł</p></div>"
  ?>
</div>
</body>
<script>
